...
export class UpdatePwdComponent implements OnInit {
    ...
    UpdatePwd() {
        var NewUserInfo = {id:this.UserInfo.id, password:this.PassWord}
        if (this.PassWord != this.SecPassWord) {
            this.message['error']('两次密码输入不一致')
            return
        }
        this.userservice.UpdatePwd(NewUserInfo).subscribe(
            fb=>{
                if (fb['code'] == 200) {
                    ...
                    this.message['error']('用户密码修改失败，服务错误')
                  }
                },
                err=>{
                    console.log(err)
                    this.message['error']('用户密码修改失败，未知错误')
                }
              )
    }
    LogOut() {
